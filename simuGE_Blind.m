...
function [simuLatency,Pr_Gn,Pr_Bn]=simuGE_Blind(lambda, p, r,alpha)
...
PackeTransmitted=GoodState.*(rand>PG)+(1-GoodState).*(rand>PB); %The packet is successfully delivered
...
for i=1:(TotalPackets-1)
    ...
    range=GenerateTime(i):(GenerateTime(i+1)-1);
    ChannelStatus=PackeTransmitted(range).*(rand(1,length(range))<=alpha);
